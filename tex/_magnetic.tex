...
c_{+}(\vK), & c_{-}(\vK), & c_{-}^† (-\vK), & -c_{+}^† (-\vK)\end{array}\right]^{T}$, as
\begin{equation}
...
\begin{equation}
\mathcal{H}_{imp}=\sum_{j}\int d^{2}\vc{r}U_{0}\delta(\vc{r}-\vc{R}_{j})ρ(\vc{r})\label{eq: impurity_H_real}
\end{equation}
...
as
\begin{equation}
\mathcal{H}_{imp}=\sum_{\vK_{1},\vK_{2}}\sum_{j}e^{i(\vK_{1}-\vK_{2})\cdot\vc{R}_{j}}\Psi_{\vK_{1}}^† \hat{U}\Psi_{\vK_{2}}\label{eq: impurity_H_momentum}
\end{equation}
